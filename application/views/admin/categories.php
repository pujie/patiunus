...
<body>
    <div id="dRemoveConfirmation" class="modal hide fade" role="dialog" aria-labelledby="myModalLabel">
        ...
        <div class="modal-body">
            <p>Apakah anda yakin hendak menghapus category <span id="categoryname2delete"></span> ?.</p>
            <button categoryid2delete="" id="btndelete">Ya</button>
        </div>
    </div>      
...
<script type="text/javascript">
    jQuery(document).ready(function() {
        ...
        $("#categories .categoryremove").click(function(){
            $.ajax({
				url:'<?php echo base_url();?>categories/getjson/'+$(this).attr('categoryid'),
				dataType: 'json',
				async:false
            }).done(function(data){
                $("#btndelete").attr("categoryid2delete",data["id"]);
				$("#categoryname2delete").text(data["name"]);
            }).fail(function(){alert("tidak bisa");});
        	$("#dRemoveConfirmation").modal();
        });
        $("#btndelete").click(function(){
			window.location.href = "<?php echo base_url();?>admin/category_remove/"+$(this).attr("categoryid2delete");
        });
        $("#btncategoryadd").click(function(){
			window.location.href = "<?php echo base_url();?>admin/category_add";
        });
    });
</script>
<!--[if lt IE 9]>
